...
{
	tess.Init(NULL, "eng");
  tess.SetVariable("tessedit_char_whitelist", "12346789");
}
...
int ArmorDetector::detectArmorNumber(const ArmorDescriptor &armor)
{
	...
	if (*out == '3')	right++;
	std::cout << "acc: "<< right*1.0/total << std::endl; 
	imshow("roi", roi);
	return 0;
}
...
void ArmorDetector::loadImg(const cv::Mat &srcImg)
{
	...
	if (_flag == ARMOR_LOCAL && _trackCnt <10000)
	{
		cv::Rect bRect = boundingRect(_targetArmor.vertex) + _roi.tl();
		cv::Rect bRectbk(bRect);
		bRect = cvex::scaleRect(bRect, Vec2f(2, 2)); //������Ϊê��Ŵ�2��
		_roi = bRect & imgBound;
		// bRectbk = cvex::scaleRect(bRectbk, Vec2f(1.1, 1.1)); //������Ϊê��Ŵ�2��
		bRectbk = bRectbk -  _roi.tl();
		_targetArmor.vertex[0] = cv::Point2f(bRectbk.x, bRectbk.y);
		_targetArmor.vertex[1] = cv::Point2f(bRectbk.x+bRectbk.width, bRectbk.y);
		_targetArmor.vertex[2] = cv::Point2f(bRectbk.x+bRectbk.width, bRectbk.y+bRectbk.height);
		_targetArmor.vertex[3] = cv::Point2f(bRectbk.x, bRectbk.y+bRectbk.height);
...
ArmorDescriptor::ArmorDescriptor(const LightDescriptor &lLight, const LightDescriptor &rLight, const int armorType, const cv::Mat &grayImg, float rotaScore, ArmorParam _param)
{
	...
	cv::Size exLSize(int(lightPairs[0].size.width), int(lightPairs[0].size.height ));
	cv::Size exRSize(int(lightPairs[1].size.width), int(lightPairs[1].size.height ));
...
void ArmorDescriptor::getFrontImg(const Mat &grayImg)
{
	using cvex::distance;
	const Point2f &
		tl = vertex[0]+Point2f(5, -20),
	   tr = vertex[1]+Point2f(-5, -20),
	   br = vertex[2]+Point2f(-5, 20),
	   bl = vertex[3]+Point2f(5, 20);
	...
	grayImg.copyTo(frontImg);
	Point2f src[4]{Vec2f(tl), Vec2f(tr), Vec2f(br), Vec2f(bl)};
	Point2f dst[4]{Point2f(.0, 0.0), Point2f(width, 0.0), Point2f(width, height), Point2f(0.0, height)};
...
double ArmorDetector::getArmorLength(ArmorDetector &Arm) 
{
	std::vector<cv::Point2f> Points = Arm.getArmorVertex();
